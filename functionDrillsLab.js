...
let nameGreeting = console.log(nameCheck('Paul'))
...
let oddChecker = console.log(thatsOdd(9))
...
let theBestMovie = bestMovie('Labyrinth')
...
//CODE HERE
function theEliminator(conts1, const2){
  for(let i = 0; i < conts1.length; i++){
    if(conts1[i] === const2){
      (conts1.splice(i, 1))  
    }
  }
...
function emailCheck(email){
  ...
  let emailWhite = emailString.trim()
  if(emailWhite.includes('@')){
    console.log("email verified")
  }else{
    console.log('must provide valid email address')
  }
}
emailCheck(" dsjfds@osdf  ")
...
//CODE HERE
function frogPurchase(money){
  return(money / 3)
}
totalFrogs = frogPurchase(9)
...
//CODE HERE
function frogPurchase2(money){
  if(money % 3 !== 0){
    return (money - money % 3) /3
  }
  return (money / 3)
}
totalFrogs = frogPurchase2(13)
console.log(totalFrogs)
...
function ascend(arr){
  ...
  for(j = arr[i] + 1; j < arr.length - 1; j++){
      if(arr[i] < arr[j]){
        return(true)
      }else if(arr[i] >= arr[j]){
        return(false)
...
let bathtubScope = ["duck", "rubber duck", "sailor duck"]
